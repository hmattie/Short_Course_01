...
**For this lab you'll be answering questions 8 and 9. We answered questions 1-7 in lecture.** 
...
```{r}
autism_prevalence_CA %>% 
    ggplot(aes(x = year, y = prevalence)) +
    geom_line(alpha = 1/2) + 
    geom_point() + 
    xlab("Year") +
    ylab("Prevalence") +
    ggtitle("Autism Prevalence in California") + 
    geom_vline(xintercept = 1963, col = "blue")
```

Note: The blue line in the plot denotes the year the Measles vaccine was introduced (1963). Other lines can be added for other vaccines.

The plot above clearly shows an increase in Autism prevalence over time. However, the Measles vaccine is introduced in 1963, and a large increase in Autism prevalence isn't seen until over 20 years later. Additionally, the prevalence of Autism itself has not increased over time - the diagnosis of Autism has increased as more clinical studies have been conducted, and the definition of Autism has evolved. All of the above makes the argument that the Measles vaccine (or any vaccine) causes Autism invalid and misleading. 
...
A complete response could include (for at least one other disease): at least one plot that displays disease rate over time and the corresponding vaccine introduction date; and a discussion of the benefits of vaccines that refers to the plot(s) generated as supporting evidence.
